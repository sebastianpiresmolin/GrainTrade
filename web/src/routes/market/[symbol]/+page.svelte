<script lang="ts">
  ...
  const position = $derived(
    account.holdings.find((h) => h.symbol === data.quote.symbol),
  );

  // My live pending orders for this symbol (store holds all symbols).
  const openOrders = $derived(
    (market.orders ?? data.orders).filter(
      (o) => o.symbol === data.quote.symbol,
    ),
  );
  ...
  const synced: SubmitFunction =
    ...
    async ({ update, result }) => {
      ...
      if (result.type === "success") {
        const acc = (
          result.data as { order?: { account: AccountSummary } } | undefined
        )?.order?.account;
...
<section class="card">
  ...
  <div class="wallet">
    ...
    {#if position}
      <span
        ...
        <small>avg {money(position.averageCost)}</small></span
      >
    {/if}
  </div>
  ...
  <form method="POST" use:enhance={synced} class="market">
    ...
    <button type="submit" formaction="?/sell" class="sell" disabled={!position}
      >Sell</button
    >
  </form>
  ...
  <form method="POST" use:enhance={synced} class="limit">
    ...
    <button
      type="submit"
      formaction="?/limitSell"
      class="sell"
      disabled={!position}>Ask</button
    >
  </form>
</section>
  ...
  <section class="card">
    ...
    <ul class="trades">
      {#each trades.slice(0, 8) as trade (trade.tradeId)}
        <li>
          <span
            class:up={trade.side === "Buy"}
            class:down={trade.side === "Sell"}>{trade.side}</span
          >
